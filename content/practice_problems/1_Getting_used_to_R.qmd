...
The : means **to make a list from the bottom number to the top number
that includes all integers in the interval**.
...
-   max(x) returns the greatest value in x

-   x\[x \< 10\] returns all values of x less than 10 (it segments the
    array based on the condition)

-   x\^2 returns the values of x, but squared

-   x\[x \< 12 & x \> 7\] applies two conditions to x, returning all
    values of x less than 12 and greater than 7 (segmentation based on
    two conditions)
...
-   -1:2 generates a sequence from -1 to 2, with a step size of 1

```{r}
(-1):2
```

-   (-1):2 generates a sequence from -1 to 2, with a step size of 1 (the
    same as -1:2)
...
-   -(1:2) generates a sequence from 1 to 2, with a step size of 1, and
    then applies a negative to all values in that sequence
...
-   In each variable, the following data types are stored in each
    variable (format is variable (data type):
...
```{r}
# Check if proper changes were made
str(sleep)
```
...
-   The plots (both unlabeled and labeled) show how the variance in
    sleep differs between primates and non-primates
...
-   The issue in the data is that there are outliers (indicated by the
    isolated dots at the right-end of the plot)

    -   Outliers can skew data and provide inaccurate representations of
        the data, so it's important to remove outliers before proceeding
        with any further analyses
...
-   The code above removes the outliers (any BrainWt \> 1000 is removed)

-   The p-value is 0.0002782, which is lower than 0.05 and thus
    indicates a statistically significant relationship between brain
    weight and total hours of sleep in this data set
...
-   Adding in a trend line shows that there is a negative relationship
    between brain weight and total sleep: as brain weight increases, a
    species sleeps for fewer hours
