...
**Explanation:** Each of these code chunks generates the expected
distribution of the signal under the null hypothesis.

\* Calculate and compare p-values obtained using

\+ simulation (calculation won’t be required on test, but make sure you
understand!) (code or written explanation)
...
```{r}
#Explanation: The p-value from all methods are <0.05, so I reject the null hypothesis that the proportion of right-eared people is equal to 0.5. The 95% 5% CI is .562 - .888, which does not include 0.5.
```
...
**Explanation:** Because the p-value is less than 0.05, I reject the
null hypothesis that the proportion of correct answers is 0.25. Since
the dog's proportion of correct answers is 0.33, there is some evidence
that the students may have helped the dog with the test. It is very
unlikely for the dog to have scored above a 25 on the test by chance
alone.
