...
-   What type of variation do we see in total time spent sleeping?
    ...
    # We see a roughly normal distribution in the total time spent sleeping.
    ```

    We see a roughly normal distribution in the total time spent
    sleeping.
...
-   95% of the time, the true population parameter will be in the
    created interval whenever it's created/generated.
...
-   https://www.zoology.ubc.ca/\~whitlock/Kingfisher/CLT.htm
    ...
    -   No matter how the data is distributed, if the sample size is
        large enough, the distribution of the means of the samples
        (which are from the entire data) will exhibit a normal
        distribution.
...
lines or segments). **Source** runs the entire codaae at once (similar
to knitting an Rmd file) without showing any console output, but graphs
and objects are still produced!
...
-   Normal(Z) (0,1) {no Kurtosis / no skewness / no truncation}
    -   Regardless of sample size, the distribution will be normal
